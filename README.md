...
- **Road-aware detours with capped stop spacing**: Safe detour paths are split into stops with ≤ 2 km spacing, snapped to nearby roads, and routed segment‑by‑segment. If a road segment fails or intersects a stage buffer, that segment falls back to the safe path (still non‑intersecting).
- **Shortest valid route selection**: Multiple detour candidates are evaluated and the shortest safe route within the attempt/time budget is chosen.
...
- A [Google Maps API key](https://developers.google.com/maps/documentation/android-sdk/get-api-key) with:
  ...
  - For Part D routing: **Directions API** and **Roads API** (for snapping stops to roads)
...
3. **Routing (Part D)**  
   The routing layer uses the Directions API (routes) and Roads API (snap stops to roads). The Directions client in `lib/features/routing/data/directions_client.dart` expects an API key (e.g. via env or config). Pass it when constructing `DirectionsClient(apiKey: '...')` if you implement the full routing UI.
...
The alternate routing system is **deterministic geometry‑based**. It builds a visibility‑graph‑style path that avoids buffered stage and closed‑road lines, then turns that safe path into a road‑following route with per‑segment fallbacks.
...
- **Detour nodes**: offset perpendicular to each stage segment (two points per segment, one per side).
- **Corridor‑focused detours**: extra detour points near the origin→destination corridor to bias toward shorter “same‑side” routes.
...
If a path is found, we return the ordered node list as a **safe base path**.

### 5) Road‑aware route construction
We then turn the safe path into a road‑following route:

- **Stop spacing cap**: split the safe path so each segment is ≤ 2 km.
- **Road snapping**: snap stops to nearby roads via the Roads API (snaps > 250 m are ignored to avoid drifting too far).
- **Segment routing**: request Directions for each stop‑to‑stop segment.
- **Safe fallback per segment**: if a road segment fails or intersects the stage buffer, fall back to the safe segment from the base path (still non‑intersecting).

### 6) Multi‑pass detour search and shortest selection
...
- The shortest safe route found within the attempt/time budget is returned.
- If needed, a widened perimeter‑node fallback is attempted to go around large barriers.

### 7) Fallback behavior
The search continues expanding detours to find a safe route. If the geometry is impossible (e.g., origin/destination trapped inside the buffer), the search can still return empty, but it never crashes the app or blocks the UI.
